                    ...
                    (let loop3 ((rows rows)
                      ...
                      (if (null? rows)
                          ...
                          (let* ((row (extract-row (car rows)))
                                 (distance (distance-to-line count (row 'low) line-poly)))
      ...
      (if (null? rows1)
          ...
          (let* ((first (extract-row (car rows1)))
            ...
            (let loop2 ((rest (cdr rows1))
              ...
              (if (null? rest)
                  (loop (cdr rows1) min-line-poly min-total-distance (+ x0 1))
                  (let ((line-poly (make-line-poly x0 (first 'high)
                                                   x1 ((extract-row (car rest)) 'high))))
                    (let loop3 ((rows rows)
                                (total-distance 0)
                                (count 0))
                      (if (null? rows)
                          (if (> min-total-distance total-distance)
                              ...
                              (loop2 (cdr rest) min-line-poly min-total-distance (+ x1 1)))
                          (let* ((row (extract-row (car rows)))
                                 (distance (distance-to-line count (row 'high) line-poly)))
                            (if (or (= count x0) (= count x1))
                                (loop3 (cdr rows) total-distance (+ count 1))
                                (if (> distance 0)
                                    (loop2 (cdr rest) min-line-poly min-total-distance (+ x1 1))
